...
public:
    ...
    //Возвращает количество синонимов
    ...
    //Проверяет слово что является синонимом
    bool AreSynonyms(const string& first_word, const string& second_word) const {
        if (synonyms_.count(first_word) != 0) return synonyms_.at(first_word).count(second_word) != 0;
        return false;
    }
...
void TestAreSynonyms() {
    ...
    assert(synonyms.AreSynonyms("music"s, "cat"s) == false);
    assert(synonyms.AreSynonyms("cat"s, "music"s) == false);
}
